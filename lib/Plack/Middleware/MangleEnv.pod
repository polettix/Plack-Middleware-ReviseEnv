...
I<sources> (i.e. C<env>, C<ENV> or C<value>).

It collects data from a list of C<sources>, optionally C<flatten>inig
them (if they are array or hash references), filtering them (e.g. to set
default values for C<undef> or empty strings, or removing unwanted
elements), then optionally C<join>inig them or passing them to
C<sprintf>. See L</generate_hash_manglers_list> for the details on this
hash and of each element in C<sources>.
...
Something similar happens with C<sprintf>, although the format is
...
In addition to plain strings, you can also set either C<join> or
C<sprintf> (never the two at the same time!) with a I<source>
specification with C<env>, C<ENV> or C<value>:
...
Exactly one of the keys C<env>, C<ENV>, C<list>, C<sub> and C<value>
