...
this method tells you whether the C<eval> interface for subroutines is
enabled or not.

The default implementation returns C<0>, i.e. a false value. This means
that the C<eval> interface is I<disabled>.

Hence, to enable the C<eval> interface, you MUST override or
monkey-patch this method to return a true value (in Perl sense) before
C<prepare_app> is called. For example, if you trust your user base you
...
=head2 B<< ALLOW_FACTORY >>

   my $perl_bool = $self->ALLOW_FACTORY();

this method tells you whether the I<factory> interface for subroutines is
enabled or not.

The default implementation returns C<0>, i.e. a false value. This means
that the I<factory> interface is I<disabled>.

Hence, to enable the I<factory> interface, you MUST override or
monkey-patch this method to return a true value (in Perl sense) before
C<prepare_app> is called. For example, if you trust your user base you
can provide the following middleware:
...
This module contains code (in subs C<__sub_from_eval> and
C<__sub_from_factory>) that can eventually lead to either an C<eval> or
to loading an arbitrary module. This code is disabled by default, but
this is not as if the code were not there.

Enabling either of C<eval> and I<factory> interfaces requires an
attacker to be able to either monkey-patch
L</ALLOW_EVAL>/L</ALLOW_FACTORY> in the main module, or to generate a
subclass where these method are overridden to provide a true value back.
Another way is to call C<__sub_from_eval> and/or C<__sub_from_factory>
directly. The L</AUTHOR> is not aware of other ways to trigger that code.
...
example, they might substitute C<prepare_app> instead and put arbitrary
code there, or if they can call C<__sub_from_eval> they might just be
able to call C<eval> directly by themselves.
...
L</ALLOW_EVAL>/L</ALLOW_FACTORY> that the L</AUTHOR> is not aware of,
whereas these ways might not be applicable to substituting
C<prepare_app> or something different instead. Or they might manage to
call C<__sub_from_eval> (or C<__sub_from_factory>) in other ways.
...
remove the code you're not comfortable with (look for C<__sub_from_eval>
and C<__sub_from_factory>), OR
