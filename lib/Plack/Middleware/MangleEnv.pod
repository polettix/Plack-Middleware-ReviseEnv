...
L</Ordering Manglers> to avoid being biten by this, but the bottom line
is: use the array-reference form and put manglers in the order you want
them evaluated.

=head2 Defining Manglers
...
the constructor for C<Plack::Middleware::MangleEnv> in different ways:
...
In the I<array reference> form, for each mangler you can provide:
...
forms, because the I<external key> will be used for ordering manglers
only (see L</Ordering Manglers>);
...
the key that will be mangled in C<$env>. It is a template itself, so it
...
Both the key and the value of a mangler are I<templates>. They are
...
the C<call> method is invoked, which should mean at each request hitting
Plack.
...
if option C<require_all> in the mangler definition is set to a (Perl)
...
=head2 Ordering Manglers

If you plan using intermediate variables for building up complex values,
you might want to switch to the I<array reference> form of the mangler
definition (see L</Defining Manglers>), because the hash-based
...
The hash keys C<1> and C<2> will be used to order manglers, so they are
set correctly now:
...
Note that the mangler definitions already contain a C<key> field, so
...
section L</Defining Manglers>.

Returns a new mangler.

It is used by L</prepare_app> to set the list of manglers that will be
...
going to do fancy things (see L</Ordering Manglers> for example).
...
L</Template rules>). This can be overridden on a per-mangler basis.
...
where C<manglers> is in the array form and C<opts> has any missing item
